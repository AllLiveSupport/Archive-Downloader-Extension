...
const performAdvancedSearch = async (tabId, query, startPage = 1, sortStr = null) => {
	...
	do {
		...
		try {
			...
			if (!response.ok) throw new Error(`Could not fetch info: ${response.status}`);
...
var processOneArticle = function (tabId) {
	let data = tabs[tabId];
...
var getDownloadUrlsFromMetadata = function (metadata, tabId) {
	var downloadUrls = [];
	var extIndexes = [];
	...
	metadata.files.forEach(file => {
		...
		var extension = updateExtensionsFromFile(file, tabId);
...
var updateExtensionsFromFile = function (file, tabId) {
	var extensionType = '.' + file.name.split('.').slice(-1);
	var ending = "" + extensionType;
	...
	var extension = data.extensions.find(f => f.ending === ending);
	if (extension) {
		extension.count++;
	} else {
		extension = {
